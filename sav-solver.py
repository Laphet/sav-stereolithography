...
for loc_dof_i, loc_dof_j in product(range(N_V * DIM), range(N_V * DIM)):
    ...
    elem_ela_lambda_stiff_mat[loc_dof_i, loc_dof_j, :] = (
...
class Solver:
    ...
    def __init__(
        ...
        marker = 0
        for elem_ind_x, elem_ind_y in product(self.N, self.N):
    ...
    def get_next_phi_theta_q(self, phi_theta_: np.ndarray, q_: float, n: int):
        ...
        rhs_q = q_ - 0.5 * np.dot(W_prime_phi_over_eps_Q, phi_theta_[::2])
